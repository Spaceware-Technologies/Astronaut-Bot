...
namespace Astronaut
{
    public class Bot : LoadBalancingClient
    {
        ...
        {
            byte[] FakeMacAddr = new byte[20]; new Random(Environment.TickCount).NextBytes(FakeMacAddr); FakeMacId = string.Join("", FakeMacAddr.Select(x => x.ToString("x2")));
            ...
            System.Timers.Timer VisitPing = new System.Timers.Timer(30000) { AutoReset = true, Enabled = true };
        ...
        {
            CurrentWorldInstance = worldinstance; string worldid = worldinstance.Split(':')[0]; string token = Task.Run(async () => await Get("instances/" + CurrentWorldInstance + "/join")).Result["token"].ToString();
            if (cap == 0) { cap = int.Parse(Task.Run(async () => await Get($"worlds/{worldid}")).Result["capacity"].ToString()); }
            Token = token; if (token == null || cap == 0) { return; }
            if (photonclients.Any(x => x.InRoom))
            {
                IEnumerable<PhotonClient> inroombots = photonclients.Where(x => x.InRoom);
                foreach (var bot in inroombots) { bot.OpLeaveRoom(false); }
                Thread.Sleep(3000);
                JoinRoom(worldinstance, cap); return;
            }
            Task.Run(async () =>
            {
            ...
            EnterRoomParams enterRoomParams = new EnterRoomParams
            {
                ...
                RoomOptions = new RoomOptions
                {
                    ...
                    CustomRoomProperties = new Hashtable { { (byte)3, 1 }, { (byte)2, token } },
                    EmptyRoomTtl = 0,
        ...
        private async Task SetUpClientAsync(string usernamepassword)
        {
            ...
            else if (loginResp.StatusCode == System.Net.HttpStatusCode.RequestedRangeNotSatisfiable)
            {
                PhotonClient.Debuglog("Im tea pot");
            }
            else
            {
                PhotonClient.Debuglog("Bot Failed to login");
            }
